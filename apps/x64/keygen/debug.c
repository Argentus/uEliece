...
void dividePolynomials(int divident[], int divisor[], int quotient[], int remainder[]) {
  //TODO we do not need to shift there, just add polynomials with offset
    ...
    // printf("%d, DIVIDENT:%d DIVISOR:%d \n", i++, degreeOfDivident, degreeOfDivisor);
    // putc('\n', stdout);
    // printArray(tempDivident, bytes);
    // putc('\n', stdout);
    // putc('\n', stdout);
    // printArray(divisor, bytes);
    // putc('\n', stdout);
